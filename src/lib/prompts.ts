/**
 * LLM system prompts for the extraction pipeline.
 * Each export is a single-responsibility constant.
 */
...
/**
 * Build the user prompt for the analysis step.
 */
...
/**
 * Build the user prompt for the spec step of a single component.
 */
export function buildSpecUserPrompt(component: { name: string; tag: string | null; html: string; cssClasses: string | null; inlineStyles: string | null }): string {
...
/**
 * Build the user prompt for the code generation step of a single component.
 */
export function buildGenerateUserPrompt(params: {
  ...
  return `Generate a ${params.codeFormat.toUpperCase()} component based on this specification:
...
${params.html}
${params.tokensContext}

${formatInstructions[params.codeFormat] || formatInstructions.html}
